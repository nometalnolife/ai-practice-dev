...
def nbmerge(stages=" ", output="nbmerge.ipynb"):

    nb_list = [stage for stage in stages.split(" ")]
...
def generated_combined_nb(nb_list):
    for i, file_name in enumerate(nb_list):
        notebook = os.path.join('stages',file_name)
        with open(notebook, mode='r', encoding='utf-8') as nb:
            ...
            if i == 0:
                ...
                if is_variable_matched(current_output_variable, next_input_variable):
...
def is_variable_matched(output_variable, input_variable):
...
def decompose_nb_stages(file_name='', output_folder='stages', markdown_header='# STAGE'):
